...
namespace
{
	...
	// Return true if the given char is an operator.
...
void Lexer::lex()
{
    ...
    for(std::size_t i(0) ; i < m_input.size() ; ++i)
    {
        ...
        if(is_token_complete)
        {
            ...
            if(current_token.type == TokenType::TT_IDENTIFIER && current_token.value == "null")
                current_token.type = TokenType::TT_NULL;
